...
(function(){
  ...
  window.kspRequireRole=function(roles){
    ...
    if(!roles.includes(s.role)){ location.href=s.role==="superadmin"?"../superadmin/dashboard.html":s.role==="admin"?"../admin/dashboard.html":"../lapangan/dashboard.html"; return null}
    return s;
  };
  window.kspLogout=async function(){
    window.kspSession.clear();
  ...
  window.kspVerifyActiveUser=async function(){
    if(authCheckRunning) return true;
    authCheckRunning=true;
    try{
      if(!window.supabaseClient) return true;
      const local=window.kspSession.get();
      if(local && Date.now()>=Number(local.expiresAt||0)){
      ...
      const {data:profile,error}=await window.supabaseClient
        ...
        .maybeSingle();
      if(error || !profile || profile.is_active!==true || !["admin","lapangan","superadmin"].includes(profile.role)){
        await window.supabaseClient.auth.signOut();
        window.kspSession.clear();
        location.href="../login.html";
        return false;
      }
      const current=window.kspSession.get();
      if(!current || current.role!==profile.role){
        window.kspSession.save(profile.role,profile.display_name||session.user.email||"");
      }
      return true;
    }catch(e){
      console.warn("Auth guard error",e);
      try{if(window.supabaseClient) await window.supabaseClient.auth.signOut()}catch(ignore){}
      window.kspSession.clear();
      location.href="../login.html";
      return false;
    }finally{authCheckRunning=false}
  };

  window.kspStartSessionGuard=function(){
    const s=window.kspSession.get(); if(!s)return;
    const check=()=>{
      if(Date.now()>=Number((window.kspSession.get()||{}).expiresAt||0)){window.kspLogout();return}
      window.kspVerifyActiveUser();
    };
    check();
    setInterval(check,30000);
  };

  // Hardens Admin/Lapangan pages that already load this shared runtime.
  // Superadmin pages use their own Supabase Auth guard and are intentionally excluded.
  const path=location.pathname.toLowerCase();
  if(/\/admin\/|\/lapangan\//.test(path)){
    setTimeout(()=>window.kspVerifyActiveUser(),0);
  }
})();
