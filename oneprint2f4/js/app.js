...
async function loadAll() {
  try {
    ...
    console.error("OnePrint: gagal memuat inventori", error);
    state.inventory = [];
    view.toast("Inventori tidak dapat dimuat", "error");
  }

  try {
    state.services = await listServices();
    console.info(`OnePrint: ${state.services.length} data servis ditemukan`);
  } catch (error) {
    console.error("OnePrint: gagal memuat servis", error);
    state.services = [];
    view.toast("Data servis tidak dapat dimuat", "error");
  }

  // Always render the UI even if one Firebase collection fails.
  renderInventory();
  renderStats();
  renderServiceTable();
}
function renderStats() {
  ...
  // Stat cards are independent: one missing optional element must never
...
function resetServiceForm(){
  ...
  $("#service-mode").textContent="Transaksi Baru"; $("#delete-service").hidden=true; $("#save-print-note").disabled=false; renderCart();
}
...
function bind(){
  ...
  $("#new-service-3")?.addEventListener("click",()=>{resetServiceForm();switchPage("kasir")});
