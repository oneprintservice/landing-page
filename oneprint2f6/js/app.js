...
function renderServiceTable() {
  ...
  const serviceList = $("#service-list") || $("#service-table-body") || document.querySelector("#page-services tbody");
  if (serviceCount) serviceCount.textContent = `${rows.length} tampil`;
  if (!serviceList) {
    console.warn("OnePrint: elemen daftar servis tidak ditemukan. Kemungkinan app.html dan app.js berbeda versi/cache.");
    return;
  }
...
function bind() {
  ...
  document.addEventListener("click", async e => {
    ...
    if (page) { view.closeModal(); switchPage(page.dataset.page); return; }
