...
function renderPrinterContext(printer, history = []) {
  ...
  el.innerHTML = `
    ...
    <div class="printer-context-grid">
      <span>ID</span><b>${escapeHtml(printer.barcode || "-")}</b>
...
async function loadPrinterIdentity(code, { notify = true } = {}) {
  ...
  if (!printer) {
    ...
    state.selectedPrinter = null;
    renderPrinterContext(null, []);
    if (notify) view.toast(`Printer ${barcode} baru. Barcode ini akan menjadi identitas printer servis dan disimpan saat transaksi disimpan.`, "info");
    return null;
  }
...
function bind() {
  ...
  const lookupSerial = $("#lookup-serial");
  if (lookupSerial) {
    lookupSerial.addEventListener("click", async () => {
      const code = $("#barcode-input").value.trim();
      if (!code) return view.toast("Masukkan atau scan ID / barcode printer terlebih dahulu.", "info");
      await loadPrinterIdentity(code);
    });
  }
  ...
  $("#lookup-serial").addEventListener("click", async () => {
    const code = $("#barcode-input").value.trim();
    if (!code) return view.toast("Masukkan atau scan ID printer terlebih dahulu", "error");
    await loadPrinterIdentity(code);
  });
