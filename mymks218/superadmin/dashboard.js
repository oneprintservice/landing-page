...
async function edge(action,payload={}){
  ...
  const r=await fetch(EDGE_FN,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${session.access_token}`},body:JSON.stringify({action,...payload})});
  let out={};try{out=await r.json()}catch{}
  if(!r.ok) throw new Error(out.error||`Edge Function HTTP ${r.status}`);
  return out;
}
...
async function saveUser(userId){
  ...
  try{await edge("update",{user_id:userId,display_name:name,role,is_active:true});window.kspNotify("Profil berhasil diperbarui.","success","User");await loadUsers();await loadAudit()}catch(e){show(e.message)}
}
