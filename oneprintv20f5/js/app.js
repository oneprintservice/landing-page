...
function renderAccounting() {
  ...
  // Data servis DIAMBIL lama yang belum memiliki ledger tetap dihitung,
  // tetapi hanya sekali berdasarkan nomor servis.
  ...
  $("#ledger-list").innerHTML = displayRows.slice(0, 150).map(x => `<tr>
    <td>${new Date(x.tanggal || 0).toLocaleDateString("id-ID")}</td>
...
function resetManualLedgerForm() {
  ...
  $("#ledger-type").value = "PEMASUKAN";
  $("#ledger-category").value = "OPERASIONAL";
...
function bind() {
  ...
  document.addEventListener("click", async e => {
    ...
    if (editLedger) {
      ...
      $("#ledger-type").value = row.tipe || "PEMASUKAN";
      $("#ledger-category").value = row.kategori || "LAINNYA";
      $("#ledger-date").value = dateInput(row.tanggal);
      $("#ledger-amount").value = Number(row.jumlah || 0);
      $("#ledger-note").value = row.keterangan || "";
      ...
      document.querySelector("#page-accounting .accounting-grid .panel")?.scrollIntoView({ behavior: "smooth", block: "start" });
    ...
    if (delLedger) {
      ...
      if (confirm("Hapus transaksi manual ini? Data yang dihapus tidak dapat dipulihkan.")) {
        try {
  ...
  $("#save-manual-ledger").addEventListener("click", async () => {
    ...
    try {
      await saveLedger({
        tanggal: date,
        tipe: $("#ledger-type").value,
        kategori: $("#ledger-category").value,
        sumber: "MANUAL",
        keterangan: note,
        jumlah: amount
      }, key);
      ...
      await loadAll();
      view.toast(isEdit ? "Transaksi manual berhasil diperbarui." : "Transaksi manual berhasil dicatat.", "success");
    } catch (err) {
      console.error("OnePrint: gagal menyimpan transaksi manual", err);
  ...
  $("#cancel-manual-ledger").addEventListener("click", () => {
    resetManualLedgerForm();
    view.toast("Edit transaksi dibatalkan.", "info");
  });
