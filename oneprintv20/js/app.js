...
async function loadPrinterIdentity(code, { notify = true } = {}) {
  ...
  if (!printer) {
    ...
    if (notify) view.toast(`Printer ${barcode} belum terdaftar. Data printer akan dibuat saat servis disimpan.`, "info");
    return null;
  }
...
function bind() {
  ...
  document.addEventListener("click", async e => {
    ...
    if (e.target.closest("#scan-printer") || e.target.closest("#scan-service-form") || e.target.closest("#scan-serial")) { startCodeScanner("printer"); return; }
