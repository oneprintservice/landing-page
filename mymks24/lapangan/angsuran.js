...
async function loadActiveLoans(){
 ...
 const {data:loans,error}=await supabaseClient.from("pinjaman").select("*").eq("status","aktif").order("created_at",{ascending:false});
...
function renderActive(){
 ...
 if(!rows.length){result.innerHTML='<section class="ksp-card"><div class="empty">Tidak ada pinjaman aktif pada tanggal/filter tersebut.</div></section>';return}
