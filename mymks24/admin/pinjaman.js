...
const now=new Date(); dateFilter.value=`${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,"0")}-${String(now.getDate()).padStart(2,"0")}`;
const esc=kspEscape;
async function load(){
 list.innerHTML='<section class="ksp-card"><p>Memuat data...</p></section>';
 let {data: loans,error}=await supabaseClient.from("pinjaman").select("*").eq("status","aktif").order("created_at",{ascending:false});
