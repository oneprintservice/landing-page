...
async function load(){
 ...
 let {data: loans,error}=await supabaseClient.from("pinjaman").select("*").order("created_at",{ascending:false});
 ...
 document.querySelectorAll(".save-schedule").forEach(b=>b.onclick=async()=>{
   ...
   b.disabled=false;kspNotify("Jadwal 12 baris berhasil disimpan.","success","Berhasil");load();
 });
}
