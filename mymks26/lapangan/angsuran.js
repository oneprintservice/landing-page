...
async function loadActiveLoans(){
 ...
 for(const p of (loans||[])){
  ...
  if(ar.error)continue; const next=(ar.data||[])[0]||null;
...
function renderActive(){
 ...
 const rows=activeLoans.filter(x=>{const n=x.n;const text=[n.nomor_anggota,n.nama_lengkap,n.nik].map(v=>String(v||"").toLowerCase()).join(" ");return (!term||text.includes(term))&&(!day||!x.next||localDate(x.next.tanggal_jatuh_tempo)===day)});
 ...
 result.innerHTML=rows.map(x=>{const n=x.n,p=x.p,a=x.next;return `<section class="ksp-card"><div class="ksp-customer"><strong>${esc(n.nama_lengkap)}</strong><div class="meta">No. Anggota: ${esc(n.nomor_anggota)} · NIK: ${esc(n.nik)}</div><div class="meta">${esc(n.alamat)}</div><div class="meta">Pinjaman: ${kspFormatRupiah(p.jumlah_pinjaman)}${a?` · Jatuh tempo: ${kspFmtDate(a.tanggal_jatuh_tempo)}`:""}</div></div>${a?`<form class="pay-form ksp-grid" data-pid="${p.id}" data-nid="${n.id}" style="margin-top:12px"><div class="ksp-field full"><label>Angsuran ke</label><select name="angsuran_id" required><option value="${a.id}">Ke-${a.angsuran_ke} · JT ${kspFmtDate(a.tanggal_jatuh_tempo)}</option></select></div><div class="ksp-field"><label>Tanggal Bayar</label><input name="tanggal_bayar" type="date" value="" required></div><div class="ksp-field"><label>Nominal Bayar (ribu)</label><input name="nominal" inputmode="numeric" placeholder="750" required></div><div class="ksp-field full"><label>Catatan</label><input name="catatan"></div><div class="ksp-field full"><button class="ksp-btn ksp-btn-success" type="submit">💾 Simpan Pembayaran</button></div></form>`:`<div class="meta" style="margin-top:12px">Belum ada angsuran belum lunas pada pinjaman ini.</div>`}</section>`}).join("");
