...
async function load(){
 txBody.innerHTML='<tr><td colspan="11">Memuat...</td></tr>';
 ...
 if(error){txBody.innerHTML='<tr><td colspan="11">Gagal mengambil transaksi.</td></tr>';kspNotify(error.message,"error","Ledger");return}
 txBody.innerHTML=data?.length?data.map((x,i)=>`<tr><td>${i+1}</td><td>${kspFmtDate(x.tanggal)}</td><td>${kspEscape(x.nomor_anggota)}</td><td>${kspEscape(x.nama)}</td><td>${kspEscape(x.alamat)}</td><td>${kspFormatRupiah(x.drop_lama)}</td><td>${kspFormatRupiah(x.drop_baru)}</td><td>${kspFormatRupiah(x.bht)}</td><td>${kspFormatRupiah(x.materai)}</td><td>${kspEscape(x.staf_nama||"-")}</td><td>${isSuperadmin?`<button type="button" class="ksp-btn ksp-btn-danger delete-ledger" data-id="${kspEscape(x.id)}" style="min-height:34px;padding:7px 10px">Hapus</button>`:"-"}</td></tr>`).join(""):'<tr><td colspan="11">Belum ada transaksi.</td></tr>';
...
async function deleteLedger(id){
 if(!isSuperadmin){return kspNotify("Hanya Superadmin yang dapat menghapus transaksi.","error","Akses ditolak")}
 const reason=prompt("Alasan menghapus transaksi ini:","Salah input");
 if(reason===null)return;
 if(!reason.trim())return kspNotify("Alasan penghapusan wajib diisi.","warning","Periksa data");
 const row=await supabaseClient.from("ledger_harian").select("*").eq("id",id).single();
 if(row.error)return kspNotify(row.error.message,"error","Transaksi tidak ditemukan");
 if(!confirm(`Hapus transaksi ${row.data.nama||"ini"} tanggal ${kspFmtDate(row.data.tanggal)}?`))return;
 const del=await supabaseClient.from("ledger_harian").delete().eq("id",id);
 if(del.error)return kspNotify(del.error.message,"error","Hapus gagal");
 const audit=await supabaseClient.rpc("ksp_write_audit",{p_action:"DELETE_LEDGER",p_target_table:"ledger_harian",p_target_id:String(id),p_metadata:{reason:reason.trim(),record:row.data}});
 if(audit.error)console.warn("Audit gagal",audit.error);
 window.kspNotify("Transaksi ledger dihapus dan dicatat di Audit Log.","success","Superadmin");
 await loadStaff(); await load();
}
txBody.addEventListener("click",e=>{const b=e.target.closest(".delete-ledger");if(b)deleteLedger(b.dataset.id)});
