...
async function loadDetailNasabah() {
    ...
    document.getElementById("nomorAnggotaNasabah").textContent = nasabah.nomor_anggota ?? "-";

    document.getElementById("nikNasabah").textContent = nasabah.nik ?? "-";
    ...
    if(document.getElementById("statusNasabahEdit")) document.getElementById("statusNasabahEdit").value=nasabah.status||"aktif";
...
// ============================================================
// SIMPAN STATUS MANUAL ADMIN
if(saveStatusBtn){saveStatusBtn.onclick=async()=>{const status=document.getElementById("statusNasabahEdit").value;const id=new URLSearchParams(location.search).get("id");const r=await supabaseClient.from("nasabah").update({status}).eq("id",id);if(r.error)kspNotify(r.error.message,"error","Gagal mengubah status");else{kspNotify("Status nasabah berhasil diubah.","success","Berhasil");document.getElementById("statusNasabah").textContent=status;}}}
