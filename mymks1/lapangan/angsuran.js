...
async function searchCustomers(){
 ...
 for(const n of data){let r=await supabaseClient.from("pinjaman").select("*").eq("nasabah_id",n.id).eq("status","aktif").order("created_at",{ascending:false}).limit(10); if(r.error)continue;
  for(const p of (r.data||[])){let a=await supabaseClient.from("angsuran").select("*").eq("pinjaman_id",p.id).eq("status","belum_bayar").order("angsuran_ke",{ascending:true}).limit(12);
   const rows=a.data||[]; if(!rows.length) continue; html+=`<section class="ksp-card"><div class="ksp-customer"><strong>${kspEscape(n.nama_lengkap)}</strong><div class="meta">No. Anggota: ${kspEscape(n.nomor_anggota||"-")} · NIK: ${kspEscape(n.nik||"-")}</div><div class="meta">${kspEscape(n.alamat||"-")}</div><div style="margin-top:6px"><span class="ksp-score ${kspScoreClass(n.credit_score)}">${Number(n.credit_score??10)}/10 · ${kspScoreLabel(n.credit_score)}</span></div></div>
...
async function pay(e){e.preventDefault();const f=e.currentTarget;const fd=new FormData(f);const id=fd.get("angsuran_id"), amount=kspNominal(fd.get("nominal"));if(!id){kspNotify("Pilih angsuran terlebih dahulu.","warning");return}if(amount<=0){kspNotify("Nominal tidak valid.","warning");return}
