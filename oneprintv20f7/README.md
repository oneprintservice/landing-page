...
## V20F6 final patch
- Manual accounting ledger normalized for legacy/new field names and monthly filtering.
- Manual income/expense entries persist in `keuangan`, can be edited/deleted, and are included in totals.
- Inventory/accounting wide tables remain horizontally scrollable on mobile.
- Inventory stock heading is kept on one clean line on mobile.
- Printed invoice table headers `NO / RINCIAN / SUBTOTAL` are centered, bold, and black.
