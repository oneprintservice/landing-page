...
import { listLedger, saveLedger, removeLedger, normalizeLedgerRow } from "./features/accounting.js";
...
async function loadAll() {
  ...
  state.ledger = tasks[3].status === "fulfilled" ? tasks[3].value.map(row => normalizeLedgerRow(row, row.key)) : [];
...
function renderAccounting() {
  ...
  const ledgerRows = state.ledger.map(row => normalizeLedgerRow(row, row.key));
  const rows = ledgerRows.filter(x => ledgerMonth(x.tanggal) === month);

  // Ledger SERVIS adalah sumber utama pemasukan saat DIAMBIL.
  // Fallback hanya dipakai bila ledger SERVIS belum pernah tercatat.
  ...
  const expenseRows = rows.filter(x => x.tipe === "PENGELUARAN");
  const income = incomeRows.reduce((sum, x) => sum + Number(x.jumlah || 0), 0);
  const expense = expenseRows.reduce((sum, x) => sum + Number(x.jumlah || 0), 0);
...
async function syncServicePayment(data, oldData) {
  ...
  const ledgerKey = `SERVIS_${String(data.nomor || data.key || "UNKNOWN").replace(/[.#$[\\]\\/]/g, "_")}`;

  // Gunakan waktu DIAMBIL sebagai tanggal pembukuan. Key deterministik
  // mencegah pemasukan servis tercatat dua kali saat transaksi diedit.
  await saveLedger({
